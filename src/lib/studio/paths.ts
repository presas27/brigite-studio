 ...
 *
 * So on Vercel the root moves to `/tmp`. That is deliberately ephemeral — the
 * database is rebuilt from `seed.ts` on every cold start, which is exactly the
 * behaviour the preview deployment wants (a demo that always opens on known
 * mock data) and exactly the behaviour production must NOT have. Durable
 * storage arrives with Convex; until then `/app` is a prototype, not a product.
 *
